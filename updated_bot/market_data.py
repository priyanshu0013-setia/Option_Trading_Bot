...
Includes integration with Upstox API for live market data.
"""
import pandas as pd
import numpy as np
import logging
...
# Import Upstox SDK
try:
    from upstox_client.api import MarketDataApi, LoginApi
...
class MarketDataFetcher:
    ...
    def __init__(self, nse_api_key: str = "", zerodha_api_key: str = "", zerodha_api_secret: str = "", 
                 upstox_api_key: str = "", upstox_api_secret: str = ""):
        ...
        self.use_sample_data = not (nse_api_key and zerodha_api_key) and not (upstox_api_key and upstox_api_secret)
        if self.use_sample_data:
            logger.warning("No API keys provided. Using sample data.")
        
        # Initialize Upstox client if credentials are available
        self.upstox_client = None
        if UPSTOX_SDK_AVAILABLE and upstox_api_key and upstox_api_secret:
            try:
                self._initialize_upstox_client()
                logger.info("Upstox client initialized successfully")
            except Exception as e:
                logger.error(f"Failed to initialize Upstox client: {str(e)}")
                self.use_sample_data = True

    def _initialize_upstox_client(self):
        """Initialize the Upstox API client."""
        if not UPSTOX_SDK_AVAILABLE:
            logger.warning("Upstox SDK not available. Install with 'pip install upstox-python'")
            return
            
        try:
            # Configure API key authorization
            configuration = Configuration()
            configuration.api_key['api_key'] = self.upstox_api_key
            configuration.api_key['api_secret'] = self.upstox_api_secret
            
            # Create API instances
            self.login_api = LoginApi(configuration)
            self.market_data_api = MarketDataApi(configuration)
            
            # Test connection
            # Note: In a real implementation, you would handle authentication flow
            # This is a simplified version for demonstration
            logger.info("Upstox client configuration complete")
        except Exception as e:
    ...
    def get_option_chain(self, symbol: str) -> pd.DataFrame:
        ...
        """
        if not self.use_sample_data and UPSTOX_SDK_AVAILABLE and self.upstox_api_key:
            try:
                return self._get_upstox_option_chain(symbol)
            except Exception as e:
                logger.error(f"Error fetching option chain from Upstox: {str(e)}")
                logger.info("Falling back to sample data")
        
        # Fallback to sample data
        return self._get_sample_option_chain(symbol)

    def _get_upstox_option_chain(self, symbol: str) -> pd.DataFrame:
        ...
        """
        if not UPSTOX_SDK_AVAILABLE:
            raise ImportError("Upstox SDK not available")
        ...
        try:
            ...
            # Get market quotes for options
            # Note: In a real implementation, you would fetch actual option chain data
            # This is a simplified version that would need to be expanded
            
            # Construct option chain DataFrame
            # For demonstration, we're creating a structure similar to the sample data
            # In a real implementation, you would transform the API response
            
            logger.info(f"Successfully fetched {symbol} option chain from Upstox")
            ...
            df['source'] = 'upstox_api'  # Mark as coming from Upstox
            return df
    ...
    def _get_instrument_key(self, symbol: str) -> str:
        ...
        return symbol_map.get(symbol.upper(), f"NSE_INDEX|{symbol}")

    def _get_expiry_dates(self, symbol: str) -> List[str]:
        """Get available expiry dates for a symbol."""
        # In a real implementation, you would fetch this from the API
        # For demonstration, we're returning sample expiry dates
        today = datetime.now()
        
        # Generate the next 4 weekly expiry dates (assuming Thursday expiry)
        expiry_dates = []
        for i in range(4):
            ...
            next_thursday = today + timedelta(days=days_until_thursday + (i * 7))
    ...
    def get_market_data(self, symbol: str, interval: str = '1d', days: int = 30) -> pd.DataFrame:
        ...
        if not self.use_sample_data and UPSTOX_SDK_AVAILABLE and self.upstox_api_key:
            try:
                return self._get_upstox_market_data(symbol, interval, days)
            except Exception as e:
                logger.error(f"Error fetching market data from Upstox: {str(e)}")
                logger.info("Falling back to sample data")
        
        # Fallback to sample data
        return self._get_sample_market_data(symbol, interval, days)

    def _get_upstox_market_data(self, symbol: str, interval: str = '1d', days: int = 30) -> pd.DataFrame:
        """Fetch historical market data from Upstox API.
        ...
        try:
            ...
            # Map interval to Upstox interval format
            interval_map = {
                '1m': 'I1',
                '5m': 'I5',
                '15m': 'I15',
            ...
            upstox_interval = interval_map.get(interval, 'D')
            
            # Calculate from_date based on days
            ...
            logger.info(f"Fetching {symbol} market data from Upstox for period {from_date_str} to {to_date_str}")
    ...
    def get_market_depth(self, symbol: str) -> Dict:
        ...
        if not self.use_sample_data and UPSTOX_SDK_AVAILABLE and self.upstox_api_key:
            try:
                return self._get_upstox_market_depth(symbol)
            except Exception as e:
                logger.error(f"Error fetching market depth from Upstox: {str(e)}")
                logger.info("Falling back to sample data")
        
        # Fallback to sample data
        return self._get_sample_market_depth(symbol)

    def _get_upstox_market_depth(self, symbol: str) -> Dict:
        ...
        try:
            ...
            logger.info(f"Fetching market depth for {symbol} from Upstox")
            
            # Since we can't make actual API calls in this environment,
            # we'll return sample data with a note that it would be real data
            depth = self._get_sample_market_depth(symbol)
            depth['source'] = 'upstox_api'  # Mark as coming from Upstox
            return depth
